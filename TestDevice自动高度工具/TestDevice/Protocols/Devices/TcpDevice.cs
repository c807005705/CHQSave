...
namespace Mobot.TestBox.Protocols
{
    public class TcpServerDevice : IDevice
    {
        ...
        public TcpServerDevice(IPAddress address, int port)
        {
        ...
        public void Start()
        {
            ...
            log.InfoFormat("TCP Device Start {0}", DateTime.Now.ToString("G"));
            log.InfoFormat("Listening on : {0}:{1}", address, port);
        ...
        private class SocketChatClient
        {
            ...
            public void SetupRecieveCallback(TcpServerDevice app)
            {
                try
                {
